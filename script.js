document.addEventListener('DOMContentLoaded', () => {
  ...
  function storyMatchesFilters(story) {
    if (activeFilters.characters.size) {
      if (!story.characters || ![...activeFilters.characters].every(c => story.characters.includes(c))) return false;
    }
    ...
    filtered.forEach((story,index) => {
      ...
      if (story.characters) {
        charIcons = story.characters.map(c => {
          ...
          return `<img class="character-icon" src="${iconSrc}" title="${char.name}" alt="${char.name}">`;
        }).join('');
      }
  ...
  sortSelect.addEventListener('change', refreshTimeline);
